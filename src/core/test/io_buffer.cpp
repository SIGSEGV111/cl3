...
namespace
{
	...
	TEST(io_buffer_TFifo, Basic_Read_Write)
	{
		...
		EXPECT_EQ(0, fifo.Remaining());
		EXPECT_EQ(17-1, fifo.Space());
		fifo.Write("test", 4);
		EXPECT_EQ(4, fifo.Remaining());
		EXPECT_EQ(17-4-1, fifo.Space());
		fifo.Read(buffer, 4);
		...
		fifo.Write("hello", 5);
		fifo.Write("world", 5);
		EXPECT_EQ(14, fifo.Remaining());
		EXPECT_EQ(17-14-1, fifo.Space());
		...
		fifo.Write("bbbb", 4);
		fifo.Write("cccc", 4);
		EXPECT_EQ(12, fifo.Remaining());
		EXPECT_EQ(17-12-1, fifo.Space());
		EXPECT_THROW(fifo.Read(buffer, 13), TSourceDryException);
		EXPECT_EQ(12, fifo.Remaining());
		EXPECT_EQ(17-12-1, fifo.Space());

		fifo.Read(buffer, 11);
		EXPECT_EQ(1, fifo.Remaining());
		EXPECT_EQ(17-1-1, fifo.Space());
		...
		fifo.Write("cccc", 4);
		fifo.Write("dddd", 4);
		EXPECT_EQ(16, fifo.Remaining());
		EXPECT_EQ(0, fifo.Space());
		EXPECT_EQ(0, fifo.Write("eeee", 4, 0));
	...
	TEST(io_buffer_TPump, BlockingSink)
	{
		...
		EXPECT_EQ(27, bs.n_items_accepted_total);
		EXPECT_TRUE(pump.HasFinished());
	}
