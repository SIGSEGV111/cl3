...
namespace cl3
{
	namespace	system
	{
		namespace	time
		{
			...
			s64		TTime::ConvertToI	(ECUnit cunit) const
			{
				if(cunit < 0)
				{
					s64 mul = 1LL;
			...
			double	TTime::ConvertToF	(ECUnit cunit) const
			{
				if(cunit < 0)
				{
					double mul = 1.0;
			...
			TTime	TTime::ConvertFrom	(ECUnit cunit, s64 value)
			{
				if(cunit < 0)
				{
					s64 div = 1LL;
			...
			TTime	TTime::ConvertFrom	(ECUnit cunit, double value)
			{
				if(cunit < 0)
				{
					double div = 1.0;
