...
#ifndef	_include_cl3_core_system_os_task_h_
#define	_include_cl3_core_system_os_task_h_
	...
	namespace	system
	{
		namespace	os
		{
			namespace	task
			{
				...
				class	TNumaNode : public IStaticCollection<TCPU*>
				{
					public:
				...
				{
					public:
						CL3PUBF	size_t	EnumProcesses	(IDynamicCollection<TProcessInfo>&) const;	//	enumerates all processes on this host and adds them to the list; returns the number of processes added
				};
				...
				class	TProcess : public TProcessInfo
				{
					public:
						...
						CL3PUBF	GETTER	const IDynamicCollection<const char*>&	CommandlineArguments	() const;	//	commandline arguments (like argv[] in main())
						CL3PUBF	GETTER	const IDynamicCollection<IThread*>&		Threads	() const;	//	list of threads that belong to this process. for the calling threads process all registered threads are listed (even dead ones), but for other processed only those the operating system knows about are listed (usually only those that are alive)
				};
				...
				class	IThread
				{
					public:
						...
						CL3PUBF	GETTER	IDynamicCollection<TCPU*>&	Affinity();	//	returns the threads CPU affinity list
