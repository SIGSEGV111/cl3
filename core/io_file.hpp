		...
		namespace	file
		{
			...
			class	CL3PUBT	TFile : public virtual collection::IStaticCollection<byte_t>
			{
				...
				protected:
					fd_t fd;
					int access;
					const system::task::synchronization::TEvent<const collection::IStaticCollection<const byte_t>, collection::TOnChangeData<const byte_t> > on_change;

				public:
					//	from IObservable
					inline	const system::task::synchronization::TEvent<const collection::IStaticCollection<const byte_t>, collection::TOnChangeData<const byte_t> >&	OnChange	() const CL3_GETTER { return on_change; }
